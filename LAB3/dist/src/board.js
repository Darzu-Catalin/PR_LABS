...
(function (GameConstants) {
    GameConstants[GameConstants["MAGIC_ROW"] = -19527] = "MAGIC_ROW";
    GameConstants[GameConstants["MAGIC_COL"] = -19527] = "MAGIC_COL";
    GameConstants[GameConstants["WAIT_MS"] = 100] = "WAIT_MS";
...
export class Board {
     ...
     */
    constructor(height, width, cards) {
    ...
    initializeBoard(cards) {
        return cards.map(row => row.map(card => ({ card, faceUp: false, controller: undefined })));
    }
    /**
     ...
     */
    static processCards(allCards, height, width) {
        const cards = [];
     ...
     */
    static validateCardPairs(cardCounts) {
    ...
    notifyAll() {
        ...
        for (const fn of ls)
            fn(this.look("")); // any look to trigger
    }
    /**
     ...
     */
    tryClaimCell(row, col, owner) {
        const k = this.cellKey(row, col);
        if (this.claimLocks.has(k))
            return false;
     ...
     */
    releaseClaim(row, col, owner) {
        const k = this.cellKey(row, col);
        if (this.claimLocks.get(k) === owner)
            this.claimLocks.delete(k);
    }
    /**
    ...
    cleanupPreviousMove(playerId) {
        ...
        if (state.hasMatch && state.firstCard !== undefined && state.secondCard !== undefined) {
            ...
            const isCardInPlayerStates = (row, col) => {
     ...
     */
    look(playerId) {
        const lines = [`${this.height}x${this.width}`];
        for (const row of this.board) {
            for (const cell of row) {
                let status = "down";
                let text = "";
